...
class _WindowsV2RayHandler():
    ...
    def start_daemon(self):
        ...
        batfile.write(
            'START "" /B %s -device tun://tun00'
            ' -proxy socks5://127.0.0.1:1080"\n'
            % self.tunproc
        )
        batfile.write('timeout /t 2\n')
        batfile.write(
            'netsh interface ip set address "tun00"'
            ' static address=10.10.10.2'
            ' mask=255.255.255.0 gateway=10.10.10.1\n'
        )
        batfile.write(
            'netsh interface ip set dns name="tun00"'
            ' static 1.1.1.1\n'
        )
        batfile.write(
            'route add %s %s metric 5\n'
    ...
    def read_v2ray_config(self):
        ...
        with open(
            path.join(
                self.MeileConfig.BASEDIR, 'v2ray_config.json'
            ),
            'r',
        ) as V2RAYFILE:
